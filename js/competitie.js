...
function renderMatchRow(m, comp) {
  ...
  return `<div class="match-row ${isCam?'cambuur-match':''} ${isRival&&isCam?'rival-match':''}"
    ...
    <div class="match-home" style="display:flex;align-items:center;justify-content:flex-end;gap:6px;cursor:pointer;${isCam&&m.homeClubId===cam?.id?'color:var(--accent-primary)':''}" onclick="event.stopPropagation();navigateToClub('${m.homeClubId}')"><span>${homeName}</span>${clubLogoHTML(homeClub,26)}</div>
    ${scoreHtml}
    <div class="match-away" style="display:flex;align-items:center;justify-content:flex-start;gap:6px;cursor:pointer;${isCam&&m.awayClubId===cam?.id?'color:var(--accent-primary)':''}" onclick="event.stopPropagation();navigateToClub('${m.awayClubId}')">${clubLogoHTML(awayClub,26)}<span>${awayName}</span></div>
  ...
  return `<div class="card mb-12">
    ...
    <table class="data-table"><thead><tr>
      ...
      return `<tr style="${c.isOwn?'background:rgba(245,197,0,0.07);font-weight:600':''}${c.highlight==='rivaal'?';border-left:2px solid var(--rival-accent)':''}">
        ...
        <td><div style="display:flex;align-items:center;gap:6px;cursor:pointer" onclick="navigateToClub('${c.id}')">${clubLogoHTML({logo:c.logo,abbr:c.abbr,name:c.name},30)}<span>${c.isOwn?'▶ ':''}${c.name}${c.highlight==='rivaal'?' <span class="badge badge-rival" style="font-size:9px">Rivaal</span>':''}${periodBadge}${cupBadge}${excludedBadge}</span></div></td>
