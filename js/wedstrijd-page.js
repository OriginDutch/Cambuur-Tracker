...
function renderWedstrijdPage(matchId) {
  ...
  <!-- Alle inhoud hieronder — de paginabreedte zelf wordt nu universeel
       geregeld via .page in app.css, aangestuurd door de contentWidth-
       voorkeur (zie applyPrefs()) -->
  ...
  <div class="wp-section" style="text-align:center;margin-bottom:16px">
    <div style="display:flex;align-items:center;justify-content:center;gap:16px;margin-bottom:8px">
      <div style="flex:1;display:flex;align-items:center;justify-content:flex-end;gap:8px;font-size:18px;font-weight:700;cursor:pointer;color:${isCamHome?'var(--accent-primary)':'var(--text-primary)'}" onclick="navigateToClub('${m.homeClubId}')"><span>${homeName}</span>${clubLogoHTML(homeClub,40)}</div>
      ...
      <div style="flex:1;display:flex;align-items:center;justify-content:flex-start;gap:8px;font-size:18px;font-weight:700;cursor:pointer;color:${!isCamHome?'var(--accent-primary)':'var(--text-primary)'}" onclick="navigateToClub('${m.awayClubId}')">${clubLogoHTML(awayClub,40)}<span>${awayName}</span></div>
    </div>
