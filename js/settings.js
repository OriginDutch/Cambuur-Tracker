  ...
  const rowsHtml = (groupKey, clubs) => {
    ...
    return clubs.map(c=>{
      ...
      return `<tr ${dragAttrs} style="${ownBg}${rivalBorder}${canDrag?'cursor:grab;':''}">
        <td>${canDrag?'<span style="color:var(--text-muted);margin-right:4px" title="Sleep om te herordenen">⠿</span>':''}<span style="display:inline-flex;align-items:center;gap:6px">${clubLogoHTML(c,25)}<strong>${c.name}</strong></span>${c.isOwnClub?' <span class="badge badge-active" style="font-size:9px">Eigen</span>':''}</td>
...
async function savePref(key, value) {
  ...
  applyPrefs();
  // Sommige voorkeuren (recente-vorm-lengte, topscorers/beschikbaarheid tonen)
  // veranderen wat er op het dashboard te zien is — die moet je dus opnieuw
  // renderen, niet alleen "toepassen" (dat regelt alleen thema/kleuren/zoom/etc).
